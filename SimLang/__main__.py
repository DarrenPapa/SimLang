...
if len(sys.argv) < 2:
    INTERPRETER = interpreter.inter_class()
    INTERPRETER.set_exit_method(lambda: print(end=""))
    INTERPRETER.remove_csgui()
    print(
        'SimLang - REPL [0.1] Might be slower due to the REPL\'s argument handling. "exit" to exit.'
    )
    print(
    ...
    while True:
        ...
        if act == "exit":
            ...
            code = ""
            print("Multi-line mode, Enter `quit` on a newline to run the code and exit.\n`stop` to cancel, `reset` to clear code.\n`run` to run code. `list` to see code.")
            iter = 0
            while True:
...
ok = interpreter.inter_class(sys.argv[1])
try:
    start = time()
